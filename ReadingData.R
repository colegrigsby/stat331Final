...
user.data <-
  merge(user.data, cps[, c("TUCASEID", "state", "PRTAGE", "PEHRACTT", "PEEDUCA")], by =
          "TUCASEID", all.x = TRUE)
...
ba_complete <-
  user.data.hours[(41 <= user.data.hours$education) &
                    (user.data.hours$education <= 43),]
...
for (i in 1:length(all)) {
  ...
  for (j in 1:length(currentlist)) {
    message(currentlist)
    summary[summary$PEEDUCA == currentlist[j],]$Education = all[i]
  }
}
