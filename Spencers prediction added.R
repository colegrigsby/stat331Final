...
ui <- fluidPage(
  ...
  tabsetPanel(
    ...
    tabPanel(
      ...
      sidebarPanel(
        textInput("nums", label = h3("Numeric Input For Prediction"), value = "Enter positive number..."),
...
server <- function(input, output) {
  ...
  output$value <- renderText({
    paste("If the value of ", input$choosePred, " is ", input$nums, " then then data suggests that the value of ", input$chooseResp, " will be ",
          round(lm(summary[,input$chooseResp]~summary[,input$choosePred])[[1]][[1]] + 
                  lm(summary[,input$chooseResp]~summary[,input$choosePred])[[1]][[2]]*as.numeric(input$nums), 3), input$chosePred)
  })
