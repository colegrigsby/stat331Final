...
ui <- fluidPage(
  ...
  tabsetPanel(
    ...
    tabPanel("Clustering",
             h1("Clustering"),
             sidebarPanel(radioButtons("chooseX", label="Choose an X variable", choices= c("Work","Education",
                                                                                           "Social","Sports","Age",
                                                                                           "WeeklyEarnings","Religious",
                                                                                           "Volunteer"), 
                                       selected="Education"),
                          
                          radioButtons("chooseY", label="Choose a Y variable", choices= c("Work","Education",
                                                                                          "Social","Sports","Age",
                                                                                          "WeeklyEarnings","Religious",
                                                                                          "Volunteer"), 
                                       selected="Education")               
             ),
             radioButtons("chooseN", label="Choose Number of Clusters",choices=c(2,3,4,5),selected=2),
             mainPanel(
               plotOutput("clusters")
             )
    ...
    tabPanel("Regression",
             ...
             sidebarPanel(radioButtons("chooseX", label="Choose an X variable", choices= c("Work","Education",
                          ...
                          radioButtons("chooseY", label="Choose a Y variable", choices= c("Work","Education",
                                                                                          ...
                                                                                          "Volunteer"), 
                                       selected="Education"),
                          radioButtons("chooseCat", label="Choose a categorical variable", choices= c("N/A","Year","Education Level","Day","Race"), 
                                       selected="N/A")
             ),
             mainPanel(
               plotOutput("plotter")
...
server <- function(input,output) {
  ...
  output$clusters <- renderPlot({
    clustering(n=input$chooseN,df=summary[1:10000,],c(input$chooseX,input$chooseY),c(input$chooseX,input$chooseY))
  }) 
  ...
  output$plotter <- renderPlot({
    ...
    ggplot(summary)+geom_point(aes_string(x=input$chooseX,y=input$chooseY),alpha=.2)
